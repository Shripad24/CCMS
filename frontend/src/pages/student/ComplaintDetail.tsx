...
import { Loader2, Send, Star, Sparkles, ChevronDown, ChevronUp, Paperclip } from "lucide-react";
...
export default function ComplaintDetail() {
  ...
  const { data: updatesData } = useQuery({
    ...
    queryFn: () => complaintsApi.getUpdates(id!),
    enabled: !!id,
  });
  ...
  const sendMutation = useMutation({
    mutationFn: () => messagesApi.sendMessage(id!, message),
    onSuccess: () => { setMessage(""); refetchMessages(); },
    onError: () => toast.error("Failed to send message"),
  });

  const rateMutation = useMutation({
    mutationFn: () => complaintsApi.submitRating(id!, { score: rating, feedback_text: feedback || undefined }),
    onSuccess: () => {
    ...
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="glass-card p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-4 pt-4 border-t border-slate-700/30">
          ...
          <div><p className="text-xs text-slate-400">Assigned To</p><p className="text-sm text-slate-200">{complaint.assigned_staff?.full_name || "Unassigned"}</p></div>
          ...
          {updates.map((u: any, i: number) => (
            <div key={u.id} className="flex gap-3">
              <div className="flex flex-col items-center">
                ...
                {i < updates.length - 1 && <div className="w-0.5 flex-1 bg-slate-700 mt-1" />}
              </div>
      ...
      {complaint.status === "RESOLVED" && (
        <div className="glass-card p-6">
          <h3 className="font-outfit font-semibold text-slate-200 mb-4">Rate Your Experience</h3>
          <div className="flex items-center gap-1 mb-4">
            {[1, 2, 3, 4, 5].map((s) => (
              <button key={s} onMouseEnter={() => setHoverRating(s)} onMouseLeave={() => setHoverRating(0)}
                ...
                <Star className={`w-8 h-8 ${(hoverRating || rating) >= s ? "fill-amber-400 text-amber-400" : "text-slate-600"} transition-colors`} />
              </button>
            ))}
            {rating > 0 && <span className="ml-2 text-sm text-slate-400">{rating}/5</span>}
          </div>
          <textarea value={feedback} onChange={(e) => setFeedback(e.target.value)}
            className="input-field min-h-[80px] resize-y mb-3" placeholder="Share your feedback (optional)" />
          <button onClick={() => rating > 0 && rateMutation.mutate()} disabled={rating === 0 || rateMutation.isPending}
