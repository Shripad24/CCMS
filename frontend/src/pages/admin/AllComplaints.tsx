...
export default function AllComplaints() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="font-playfair text-2xl font-bold" style={{ color: "var(--text-heading)" }}>All Complaints</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead><tr className="border-b" style={{ borderColor: "var(--divider)" }}>
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Ref</th>
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Title</th>
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Student</th>
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Staff</th>
                ...
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Priority</th>
                <th className="text-left px-4 py-3.5 text-[10px] font-mono font-bold uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>SLA</th>
              ...
              <tbody>
                {complaints.map((c: any) => (
                  <tr key={c.id} className="border-b hover:bg-white/[0.03] transition-colors" style={{ borderColor: "var(--divider)" }}>
                    <td className="px-4 py-3.5 text-sm font-mono font-medium cursor-pointer" style={{ color: "var(--accent-primary)" }} onClick={() => navigate(`/admin/complaints/${c.id}`)}>{c.reference_no}</td>
                    <td className="px-4 py-3.5 text-sm truncate max-w-[150px] cursor-pointer" style={{ color: "var(--text-heading)" }} onClick={() => navigate(`/admin/complaints/${c.id}`)}>{c.title}</td>
                    <td className="px-4 py-3.5 text-sm" style={{ color: "var(--text-body)" }}>{c.student?.full_name || "—"}</td>
                    <td className="px-4 py-3.5 text-sm" style={{ color: "var(--text-body)" }}>{c.assigned_staff?.full_name || "—"}</td>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setAssignModal(null)}>
          <div className="glass-card p-6 w-full max-w-md animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-playfair text-xl font-bold" style={{ color: "var(--text-heading)" }}>Assign Complaint</h3>
              <button onClick={() => setAssignModal(null)} className="p-1 hover:bg-white/10 rounded-lg"><X className="w-5 h-5" style={{ color: "var(--text-muted)" }} /></button>
            </div>
